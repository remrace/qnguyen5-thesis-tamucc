...
def getData():
    ...
    plt.imsave('original.png',data[0][0])
    plt.imsave('compressed.png',data[0][0][0:MAX_IDX:REDUCTION,0:MAX_IDX:REDUCTION,:])
    plt.imsave('smoothed.png',XT[0, :, :, :])
    plt.imsave('gt.png',np.squeeze(YT[0, :, :,:]), cmap='tab20')
    return X, Y, XT, YT
...
def runExp(id=0): 
        ...
        if (epoch+1) % 10 == 0:
            ...
            for i in range(Y_train.shape[0]):
                ...
                #    debugImages(YT[0,:,:,0], v_pred_img)
                    _save(YT[0,:,:,0], v_pred_img, fname='03-15/epoch_no_inference'+'_'+str(epoch+1)+'_'+str(i))
            ...
            model.save_weights('03-15/2D_diff_no_inference_0.05_' + str(epoch+1) + '_' + str(val_rand_kruskal[-1]) + '.h5')
    ...
    del model
    with open('03-15/2D_diff_no_inference_0.05_' + str(id) + '.log', 'w', newline='') as csvfile:
