...
if __name__ == '__main__':
    ...
    model_checkpoint_0 = ModelCheckpoint('C:/model/01-16-no-inference.hdf5', monitor='val_loss',verbose=1, save_weights_only=True)

    model_1 = model.unet(USE_CC_INFERENCE=True)
    model_checkpoint_1 = ModelCheckpoint('C:/model/01-16-with-inference.hdf5', monitor='val_loss',verbose=1, save_weights_only=True)
    ...
    history_0 = model_0.fit({'input_image': data[0], 'input_nlabels': data[1]}, data[2],
                    batch_size=1, epochs=500, verbose=1, validation_split=0.5, callbacks=[model_checkpoint_0], shuffle=True)
    print('USE_CC_INFERENCE: YES')
    history_1 = model_1.fit({'input_image': data[0], 'input_nlabels': data[1]}, data[2],
                    batch_size=1, epochs=500, verbose=1, validation_split=0.5, callbacks=[model_checkpoint_1], shuffle=True)
    
    print('Done training')

    with open('C:/model/01-16-history.hdf5', 'wb') as file_pi:
        pickle.dump([history_0.history, history_1.history], file_pi)
    
    with open('C:/model/01-16-history.hdf5', 'rb') as file_pi:
